...
public class RedisClient {
    ...
    public RedisClient(String host, Integer port, int db, String password, String qout, String qin) {
        this.qout = qout;
        this.qin = qin;
        if (port != null && port != -1) {
            this.port = port;
    ...
    public RedisClient(String host, Long port, Long db, String password, String qout, String qin) {
        this(host, port != null ? port.intValue(): null, db != null ? db.intValue(): 0, password, qout, qin);
    }

    public RedisClient(String host, Integer port, int db, String password, String qin) {
        this(host, port, db, password, null, qin);
    }
    
    public RedisClient(String host, Integer port, int db, String password) {
        this(host, port, db, password, null, null);
    }
    ...
    /**
     * torna il valore della chiave passata in bytes
     *
     * @param key nome della list
     * @return il valore della chiave passata
     */
    public byte[] getKeyBytes(byte[] key) {
        try (Jedis j = jp.getResource()) {
            return j.get(key);
        }
    }
     ...
     * @return il numero di valori rimossi
     */
    public long lrem(byte[] queue, byte[] value) {
        try (Jedis j = jp.getResource()) {
            return j.lrem(queue, 0, value);
        }
    }
...
//                lpopValue = lpop.get(0);
     ...
     */
    public Long expire(byte[] key, long seconds) throws Exception {
        try (Jedis j = jp.getResource()) {
            return j.expire(key, seconds);
        }
    }  
